...
int main(int argc, char* argv[]){
    ...
    //Gets the number of ways you can beat the record
    unsigned long it_maxDistance, it_higherDistance, nbWaysToWinRace;
    double maxRecord;
    //Calculates the maximum distance that can be travelled : if the record equals this value, then the record can't be beaten
    maxRecord = (raceTime * raceTime) / 4.0;
    if((raceTime % 2) == 1)
        maxRecord -= 0.25;
    if(distanceRecord == (int)maxRecord)
        nbWaysToWinRace = 0;
    else{
        //Calculates the first iteration where the record is beaten
        it_higherDistance = (int)((raceTime) - pow(((raceTime * raceTime) - (4 * distanceRecord)), 0.5)) >> 1;
        ++it_higherDistance;
        //Calculates the iteration where the maximum distance is going to be reached
        it_maxDistance = raceTime >> 1;
        //Calculates the number of ways you can beat the distanceRecord on this race
        nbWaysToWinRace = ((it_maxDistance - it_higherDistance) + 1) << 1;
    ...
    //Prints the number
    printf("\033[1;37mIn total, you have \033[1;34m%ld\033[1;37m different ways to win all the races.\033[0m\n", nbWaysToWinRace);

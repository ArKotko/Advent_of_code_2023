...
unsigned short determineType(char* cards){
    ...
    Label tabLabels[5];
    short nbLabels = 0;
    unsigned short alreadyInHand = 0, nbJokers = 0;
    unsigned short type;
    ...
    //If there is only one label, the type can only be 6
    if(nbLabels == 1)
        type = 6;
    
    //If there are jokers in the current hand
    if(nbJokers){

        switch (nbLabels){
        //If there are two labels, the best type can only be 6
        case 2 :
            type = 6;
            break;
        //If there are four labels, the best type can only be 3
        case 4 :
            type = 3;
            break;
        //If there are five labels, the best type can only be 1
        case 5 :
            type = 1;
            break;
        case 3 :
            if((nbJokers == 1) && (tabLabels[0].appearances == 2))
                type = 4;
            else
                type = 5;
        }
    }
    //If there are no jokers in the current hand
    else{
...
short compareLabels(char firstLabel, char secondLabel){
    ...
    //If one of the labels or both labels are 'J'
    if((firstLabel == 'J') || (secondLabel == 'J')){
        if((firstLabel == 'J') && (secondLabel == 'J'))
            return 0;
        else if(firstLabel == 'J')
            return -1;
        else
            return 1;
    }
    //If both labels are letters
    else if(((firstLabel >= 'A') && (firstLabel <= 'Z')) && ((secondLabel >= 'A') && (secondLabel <= 'Z'))){

        char letters[] = {'T', 'Q', 'K', 'A'};
